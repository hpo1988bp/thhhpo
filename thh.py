...
plugin = Plugin()
addon = xbmcaddon.Addon("plugin.video.thhhpo.playlist")
pluginrootpath = "plugin://plugin.video.thhhpo.playlist"
...
def GetSheetIDFromSettings():
	sid = "1yM8xk7J1zXV7PzOi0M-u9qk-H8lbeRQu8udF1wmIEZ8"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.thhhpo.playlist/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.thhhpo.playlist(/.+?/).+?\://", item["path"])
			...
			if "spreadsheets/d/" in item["path"]:
				# https://docs.google.com/spreadsheets/d/1yM8xk7J1zXV7PzOi0M-u9qk-H8lbeRQu8udF1wmIEZ8/edit#gid=0
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.thhhpo.playlist" in item["path"]:
...
def get_playable_url(url):
	if "youtube" in url:
		...
		get_thvl = "https://docs.google.com/spreadsheets/d/1yM8xk7J1zXV7PzOi0M-u9qk-H8lbeRQu8udF1wmIEZ8/export?format=tsv&gid=1287121588"
		try:
			(resp, content) = http.request(
				get_thvl, "GET"
			)
		...
		get_sphim = "https://docs.google.com/spreadsheets/d/1yM8xk7J1zXV7PzOi0M-u9qk-H8lbeRQu8udF1wmIEZ8/export?format=tsv&gid=1082544232"
		try:
			(resp, content) = http.request(
				get_sphim, "GET"
			)
